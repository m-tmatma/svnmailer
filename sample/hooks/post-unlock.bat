@echo off
REM post unlock hook -- called if a file is unlocked by a user
...
REM the file names which were unlocked.
...
"%PYTHON%" "%MAILER%" --unlock --config "%CONFIG%" --repository "%REPOS%" --author "%USER%" --background
